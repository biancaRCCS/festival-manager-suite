...
router.get("/dashboard/summary", requireStaff, async (req, res): Promise<void> => {
  ...
  const vendorPrice = settingsRows[0] ? parseFloat(settingsRows[0].vendorPrice) : 200;
  const sponsorPrice = settingsRows[0] ? parseFloat(settingsRows[0].sponsorPrice) : 500;

  const paidVendors = vendors.filter(v => v.status === "paid" || v.status === "final_approved").length;
  const paidSponsors = sponsors.filter(s => s.status === "paid" || s.status === "final_approved").length;
  const totalRevenue = paidVendors * vendorPrice + paidSponsors * sponsorPrice;
...
router.get("/dashboard/financials", requireStaff, async (req, res): Promise<void> => {
  ...
  const vendorRevenue = vendors.length * vendorPrice;
  const sponsorRevenue = sponsors.length * sponsorPrice;
